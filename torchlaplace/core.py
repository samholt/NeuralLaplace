...
def laplace_reconstruct(
    ...
    options=None,
    compute_deriv=False, # pylint: disable=unused-argument
    x0=None, # pylint: disable=unused-argument
) -> Tensor:
  ...
  batch_dim = p.shape[0]
  s, T = ilt.compute_s(torch.squeeze(t)) # pylint: disable=invalid-name
  if use_sphere_projection:
    ...
    ss = sr.view(-1, time_dim, recon_dim, s_terms_dim)
  if len(t.shape) == 2 and batch_dim == t.shape[0]:
    return ilt.line_integrate_all_multi_batch_time(ss, t.view(-1, time_dim),
                                                   T.view(-1, time_dim))
  else:
...
def _check_inputs(
  ...
  if ilt_algorithm not in ILT_ALGORITHMS:
    raise ValueError(
        f'Invalid ILT algorithm "{ilt_algorithm}". Must be one of ' + \
          '", "'.join(ILT_ALGORITHMS.keys()) + '".'
    )
  if (ilt_reconstruction_terms % 2 == 0) and (ilt_algorithm != "cme"):
    raise ValueError(
        "Invalid 'ilt_reconstruction_terms', must be an odd input number. "
        f"Was given an even number of {ilt_reconstruction_terms}")
  if options is None:
    options = {}
  else:
