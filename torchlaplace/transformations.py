"""
This module provides functions to transform between spherical Riemann
  stereographic projection coordinates and complex number coordinates.
...
def spherical_riemann_to_complex(theta, phi):
  r"""Spherical Riemann stereographic projection coordinates to complex number coordinates. I.e. inverse Spherical Riemann stereographic projection map.
    ...
    """
  # dim(phi) == dim(theta), takes in spherical co-ordinates returns complex
...
def complex_to_spherical_riemann(s_real, s_imag):
  ...
  # din(s_real) == dim(s_imag), takes in real & complex parts returns
...
def spherical_to_complex(theta, phi):
  ...
  # dim(phi) == dim(theta), takes in spherical co-ordinates returns complex
  # real & imaginary parts
  s_real, s_imag = spherical_riemann_to_complex(torch.flatten(theta),
                                                torch.flatten(phi))
...
def complex_to_spherical(s):
  ...
  # dim(phi) == dim(theta), takes in spherical co-ordinates returns complex
  # real & imaginary parts
  theta, phi = complex_to_spherical_riemann(torch.flatten(s.real),
...
if __name__ == "__main__":
  ...
  phi_outer = torch.linspace(-torch.pi / 2 + EPS, torch.pi / 2 - EPS,
                       angle_samples).double()
  print("phi ", phi_outer)
  theta_outer = torch.linspace(0 + EPS, torch.pi - EPS, angle_samples).double()
  print("theta ", theta_outer)

  s_real_outer, s_imag_outer = spherical_riemann_to_complex(theta_outer, phi_outer)

  print("s_real ", s_real_outer)
  print("s_imag ", s_imag_outer)

  theta_r_outer, phi_r_outer = complex_to_spherical_riemann(s_real_outer, s_imag_outer)

  print("theta_r ", theta_r_outer)
  print("phi_r ", phi_r_outer)

  print("Seperate tests\n\n")

  # Inf point
  print("Point at infty")
  s_real_outer = torch.Tensor([torch.inf]).double()
  s_imag_outer = torch.Tensor([torch.inf]).double()
  theta_r_outer, phi_r_outer = complex_to_spherical_riemann(s_real_outer, s_imag_outer)
  ...
  # # Zero point
  print("Point at zero")
  s_real_outer = torch.Tensor([0]).double()
  s_imag_outer = torch.Tensor([0]).double()
  theta_r_outer, phi_r_outer = complex_to_spherical_riemann(s_real_outer, s_imag_outer)
