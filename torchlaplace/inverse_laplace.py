...
"""
This module provides various classes and functions for performing Inverse
Laplace Transform (ILT) using different numerical algorithms. The ILT is used
to reconstruct trajectories for a system of Laplace representations.

Classes:
  InverseLaplaceTransformAlgorithmBase: Base class for ILT algorithms.
  FixedTablot: Implements the Fixed Tablot method for ILT.
  Stehfest: Implements the Stehfest method for ILT.
  DeHoog: Implements the DeHoog method for ILT.
  Fourier: Implements the Fourier method for ILT.
  Gaver: Implements the Gaver method for ILT.
  Euler: Implements the Euler method for ILT.
  CME: Implements the Concentrated Matrix Exponential (CME) method for ILT.

Functions:
  real_vector_to_complex: Converts a real-valued vector to a complex-valued
    vector.
  complex_numpy_to_complex_torch: Converts a complex-valued numpy array to a
    complex-valued torch tensor.

Constants:
  EPS: Machine epsilon for floating-point precision.
  TORCH_FLOAT_DATATYPE: Default torch float datatype.
  TORCH_COMPLEX_DATATYPE: Default torch complex datatype.

Usage:
  This module can be used to perform ILT on Laplace representations using
...
class FixedTablot(InverseLaplaceTransformAlgorithmBase):
  ...
  def forward(self, fs, ti, time_max=None):
    ...
    if time_max is not None:
      ...
      f0 = (torch.exp(
          (self.rdt * t) / time_max) * fs(self.rdt / self.t_time_max) / 2.0)
      fp = torch.reshape(fs(torch.flatten(s)), s.shape)
      pans = (torch.exp((self.rdt * t) / time_max).view(-1, 1) * fp *
              (1.0 + 1j * self.theta * (1.0 +
                                        (1 / torch.tan(self.theta)**2)) - 1j *
  ...
  def compute_s(self, ti, time_max=None):
    ...
    r"""Computes :math:`\mathbf{s}` to evaluate the Laplace representation :math:`\mathbf{F}(\mathbf{s})` at, from the input time points :math:`t`, using the selected ILT algorithm.
        ...
        """
    # Split of forward into generating s values, then taking function
    # evaluations at s points and integrating along that line
    t = real_vector_to_complex(ti)
    if time_max is not None:
      self.t_time_max = (real_vector_to_complex(
          torch.ones(
  ...
  def line_integrate(self, fp, ti, time_max=None):
    ...
    if time_max is not None:
      ...
      pans = (torch.exp((self.rdt * t) / time_max).view(-1, 1) * fp[:, 1:] *
              (1.0 + 1j * self.theta * (1.0 +
                                        (1 / torch.tan(self.theta)**2)) - 1j *
               (1 / torch.tan(self.theta))))
      result = (2.0 / 5.0) * (torch.sum(pans, 1) + f0) * (1 / time_max)
...
class DeHoog(InverseLaplaceTransformAlgorithmBase):
  ...
  def compute_fixed_s(self, time_max):
    ...
    s = (gamma + 1j * torch.pi * torch.arange(
        NT, dtype=self.torch_float_datatype, device=torch.device(device)) / T)
    return s
  ...
  def fixed_line_integrate(self, fp, ti, time_max):
    ...
    for r in range(1, M + 1):
      ...
      if r == 1:
        ...
        e[0:mr] = qs[r - 1][1:mr + 1] - qs[r - 1][0:mr] + es[r - 2][1:mr + 1]
        es.append(e)
      if not r == M:
        q = torch.zeros(
            (2 * M,),
        ...
        q[0:mr] = qs[r - 1][1:mr + 1] * es[r - 1][1:mr + 1] / es[r - 1][0:mr]
        qs.append(q)
  ...
  def compute_s(self, ti, time_max=None, Ti=None):
    ...
    if time_max is not None:
      ...
      tmax = tv
    if Ti is not None:
      if not isinstance(Ti, self.torch_float_datatype):
    ...
    si = torch.matmul(
        1 / T.view(-1, 1),
        1j * torch.pi * torch.arange(NT,
                                     dtype=self.torch_float_datatype,
                                     device=torch.device(device)).view(1, -1),
    )
    return gamma.view(-1, 1) + si, T

  def line_integrate(self, fp, ti, T=None):
    # pylint: disable=line-too-long
    r"""Reconstruct trajectories :math:`\mathbf{x}(t)` for `fp`, Laplace representations evaluated at `s` points from the input `ti` points, :math:`t`, using the selected ILT algorithm.
...
class Fourier(InverseLaplaceTransformAlgorithmBase):
  ...
  def line_integrate_all_multi_batch_time(self, fp, ti, T):
    ...
    t = real_vector_to_complex(ti)  # Could be slowing down ?
    gamma = self.alpha - torch.log(self.tol) / (self.scale * T)
    return (((1 / T) * torch.exp(gamma * t)).view(fp.shape[0], fp.shape[1], 1) *
            (fp[:, :, :, 0] / 2.0 + torch.sum(
                fp[:, :, :, 1:] * torch.exp(
                    torch.matmul(
                        (t / T).view(-1, 1),
...
class Gaver(InverseLaplaceTransformAlgorithmBase):
  ...
  def __init__(
    ...
    for k in range(1, max_fn_evals + 1):
      ...
      for j in range(np.floor((k + 1) / 2).astype(np.int32), min(k, ndiv2) + 1):
        inside_sum += np.exp((ndiv2 + 1) * np.log(j) - logsum[ndiv2 - j] +
                             logsum[2 * j] - 2 * logsum[j] - logsum[k - j] -
                             logsum[2 * j - k])
  ...
  def line_integrate_all_multi_batch_time(self, fp, ti, T): # pylint: disable=unused-argument
    # pylint: disable=line-too-long
    r"""Reconstruct trajectories :math:`\mathbf{x}(t)` for `fp`, Laplace representations evaluated at `s` points from the input `ti` points, :math:`t`, using the selected ILT algorithm (takes batch input of `fp`).
...
class Euler(InverseLaplaceTransformAlgorithmBase):
  ...
  def __init__(
    ...
    for k in torch.arange(1, n_euler, device=torch.device(device)):
      eta[2 * n_euler -
          k] = eta[2 * n_euler - k +
                   1] + torch.exp(logsum[n_euler - 1] - n_euler * torch.log(
                       torch.tensor(
                           2.0,
                           dtype=self.torch_float_datatype,
                           device=torch.device(device),
...
class CME(InverseLaplaceTransformAlgorithmBase):
  ...
  def __init__(
    ...
    eta = (np.concatenate((
        [params["c"]],
        np.array(params["a"]) + 1j * np.array(params["b"]),
    )) * params["mu1"])
    beta = (np.concatenate(
...
def test_all_inverse_laplace_transform_algorithms():
  ILT_RECONSTRUCTION_TERMS = 33
  ...
  print("FixedTablot Loss:"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t\t| time: {time() - t0}")

  # Split evaluation of s points out from that of the line integral
  # (should be the exact same result as above)
  decoder = FixedTablot(
      ilt_reconstruction_terms=ILT_RECONSTRUCTION_TERMS).to(device)
  t0 = time()
  ...
  print("FixedTablot Loss (Split apart):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  print(
      "FixedTablot Loss (Split apart, Fixed Max Time):"
      f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
      f"\t| time: {time() - t0}")

  # Evaluate s points for one fixed time, maximum time (Less accurate,
  # maybe more stable ?)
  ...
  print("FixedTablot Loss (Fixed Max Time):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  fh = fs(s)
  f_hat_t = decoder.line_integrate(fh, t)
  print("Stehfest Loss (Split apart):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  t0 = time()
  f_hat_t = decoder(fs, t)
  print("Fourier (Un accelerated DeHoog) Loss:"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  print("Fourier (Un accelerated DeHoog) Loss (Split apart):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  print("DeHoog Loss (Fixed Line Integrate):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
  ...
  t0 = time()
  f_hat_t = decoder(fs, t, time_max=torch.max(t))
  print("DeHoog Loss (Fixed Max Time):"
        f"\t{np.sqrt(torch.nn.MSELoss()(ft(t), f_hat_t).cpu().numpy())}"
        f"\t| time: {time() - t0}")
...
if __name__ == "__main__":
  test_all_inverse_laplace_transform_algorithms()
