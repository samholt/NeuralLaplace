"""
This module provides utility functions and classes for handling and processing
time series data in PyTorch. It includes functions for data normalization,
batching, splitting, and loss computation, as well as neural network components
for variational autoencoders and ODE-RNNs.
Functions:
- basic_collate_fn: Collate function for basic batching.
- variable_time_collate_fn: Collate function for variable time series data.
- split_and_subsample_batch: Splits and subsamples batches for training and
  testing.
- split_last_dim: Splits the last dimension of a tensor.
- init_network_weights: Initializes network weights.
- get_device: Returns the device of a tensor.
- sample_standard_gaussian: Samples from a standard Gaussian distribution.
- split_train_val_test: Splits data into training, validation, and test sets.
- linspace_vector: Creates a linearly spaced vector.
- reverse: Reverses a tensor.
- create_net: Creates a neural network.
- get_dict_template: Returns a template dictionary for data.
- normalize_data: Normalizes data.
- normalize_masked_data: Normalizes masked data.
- shift_outputs: Shifts outputs for sequence prediction.
- split_data_extrap: Splits data for extrapolation tasks.
- split_data_interp: Splits data for interpolation tasks.
- add_mask: Adds a mask to the data dictionary.
- compute_loss_all_batches: Computes loss for all batches.
- check_mask: Checks the validity of a mask.
- create_classifier: Creates a classifier network.
- get_mask: Extracts the mask from data.
Classes:
- VAE_Baseline: Variational Autoencoder baseline model.
- Encoder_z0_ODE_RNN: Encoder using ODE-RNN for latent space encoding.
...
def variable_time_collate_fn(
  ...
  """
    Expects a batch of time series data in the form of (record_id, tt, vals,
      mask, labels) where
            - record_id is a patient id
            - tt is a 1-dimensional tensor containing T time values of
              observations.
            - vals is a (T, D) tensor containing observed values for D
              variables.
            - mask is a (T, D) tensor containing 1 where values were observed
              and 0 otherwise.
            - labels is a list of labels for the current patient, if labels are
              available. Otherwise None.
    Returns:
            combined_tt: The union of all time observations.
            combined_vals: (M, T, D) tensor containing the observed values.
            combined_mask: (M, T, D) tensor containing 1 where values were
              observed and 0 otherwise.
    """
  D = batch[0][2].shape[1] # pylint: disable=invalid-name
  ...
  combined_tt = torch.stack(
      [
          torch.cat([x[1], torch.zeros(max_len - length)], 0)
          for x, length in zip(batch, lens)
      ],
      0,
  ).double()
  combined_vals = torch.stack(
      [
          torch.cat([x[2], torch.zeros(max_len - length, D)], 0)
          for x, length in zip(batch, lens)
      ],
      0,
  ).double()
  combined_mask = torch.stack(
      [
          torch.cat([x[3], torch.zeros(max_len - length, D)], 0)
          for x, length in zip(batch, lens)
      ],
      0,
  ).double()
...
def compute_loss_all_batches(
    ...
    n_traj_samples=3,
    kl_coef=1.0,
    max_samples_for_eval=None, # pylint: disable=unused-argument
):
  total = {}
  total["loss"] = 0
  total["likelihood"] = 0
  ...
  if classify:
    if data == "physionet":
      ...
      if torch.sum(all_test_labels) != 0.0:
        ...
        import sklearn as sk  # pylint: disable=import-outside-toplevel
...
class VAE_Baseline(nn.Module): # pylint: disable=invalid-name
  ...
  def compute_all_losses(self, batch_dict, n_traj_samples=1, kl_coef=1.0):
    ...
    results["mse"] = torch.mean(mse_value).detach()
...
class Encoder_z0_ODE_RNN(nn.Module): # pylint: disable=invalid-name
  """
  Encoder_z0_ODE_RNN is a neural network module that encodes input sequences into a latent space using an LSTM cell
  and a differential equation solver. The encoded latent space is then transformed to obtain the mean and standard
  deviation of the initial latent state z0.

  Args:
    latent_dim (int): Dimensionality of the latent space.
    input_dim (int): Dimensionality of the input data.
    z0_diffeq_solver (callable, optional): Differential equation solver for the latent state. Default is None.
    z0_dim (int, optional): Dimensionality of the initial latent state z0. If None, it is set to latent_dim. Default is None.
    n_gru_units (int, optional): Number of units in the GRU. Default is 100.
    device (torch.device, optional): Device to run the computations on. Default is CPU.

  Attributes:
    z0_dim (int): Dimensionality of the initial latent state z0.
    lstm (nn.LSTMCell): LSTM cell for encoding the input data.
    z0_diffeq_solver (callable): Differential equation solver for the latent state.
    latent_dim (int): Dimensionality of the latent space.
    input_dim (int): Dimensionality of the input data.
    device (torch.device): Device to run the computations on.
    extra_info (None): Placeholder for additional information.
    transform_z0 (nn.Sequential): Neural network for transforming the latent state to obtain mean and standard deviation.

  Methods:
    forward(data, time_steps, run_backwards=True, save_info=False):
      Encodes the input data into the latent space and returns the mean and standard deviation of the initial latent state z0.

    run_odernn(data, time_steps, run_backwards=True):
      Runs the ODE-RNN to encode the input data into the latent space.
  """
...
def compute_binary_CE_loss(label_predictions, mortality_label): # pylint: disable=invalid-name
  ...
  if (torch.sum(mortality_label == 0.0) == 0 or
      torch.sum(mortality_label == 1.0) == 0):
    print(
        "Warning: all examples in a batch belong to the same class -- please "
