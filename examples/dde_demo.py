...
# This file depends on the additional packages to be installed (see setup.py or documentation to install all packages) to sample from a DDE
# Note sampling a DDE is a slow process, therefore the dataset function uses memoization to keep the sampled trajectories for next run time, if applicable
# Sampling 1,000 trajectories can take approx. 15 minutes
import argparse
import logging
...
def visualize(tp_to_predict, predictions, data_to_predict, path_run_name, epoch):
    ...
    ax_top_one.plot(y_true.cpu().numpy()[0, :, 0], y_true.cpu().numpy()[0, :, 1], "k--")
    ax_top_one.plot(
        predictions.cpu().numpy()[0, :, 0], predictions.cpu().numpy()[0, :, 1], "b-"
    )
    ax_top_one.set_xlim(
        y_true.cpu()[0, :, 0].min() * margin, y_true.cpu()[0, :, 0].max() * margin
    )
    ax_top_one.set_ylim(
        y_true.cpu()[0, :, 1].min() * margin, y_true.cpu()[0, :, 1].max() * margin
    )
    ax_bottom_one.cla()
    ...
    ax_bottom_one.plot(
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[0, :, 0],
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[0, :, 1],
    )
    ax_bottom_one.set_xlim(tp_to_predict.cpu().min(), tp_to_predict.cpu().max())
    ax_bottom_one.set_ylim(
        y_true.cpu()[0, :, :].min() * margin, y_true.cpu()[0, :, :].max() * margin
    )
    ...
    ax_top_two.plot(y_true.cpu().numpy()[1, :, 0], y_true.cpu().numpy()[1, :, 1], "k--")
    ax_top_two.plot(
        predictions.cpu().numpy()[1, :, 0], predictions.cpu().numpy()[1, :, 1], "b-"
    )
    ax_top_two.set_xlim(
        y_true.cpu()[1, :, 0].min() * margin, y_true.cpu()[1, :, 0].max() * margin
    )
    ax_top_two.set_ylim(
        y_true.cpu()[1, :, 1].min() * margin, y_true.cpu()[1, :, 1].max() * margin
    )
    ax_bottom_two.cla()
    ...
    ax_bottom_two.plot(
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[1, :, 0],
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[1, :, 1],
    )
    ax_bottom_two.set_xlim(tp_to_predict.cpu().min(), tp_to_predict.cpu().max())
    ax_bottom_two.set_ylim(
        y_true.cpu()[1, :, :].min() * margin, y_true.cpu()[1, :, :].max() * margin
    )
    ...
    ax_top_three.plot(
        y_true.cpu().numpy()[2, :, 0], y_true.cpu().numpy()[2, :, 1], "k--"
    )
    ax_top_three.plot(
        predictions.cpu().numpy()[2, :, 0], predictions.cpu().numpy()[2, :, 1], "b-"
    )
    ax_top_three.set_xlim(
        y_true.cpu()[2, :, 0].min() * margin, y_true.cpu()[2, :, 0].max() * margin
    )
    ax_top_three.set_ylim(
        y_true.cpu()[2, :, 1].min() * margin, y_true.cpu()[2, :, 1].max() * margin
    )
    ax_bottom_three.cla()
    ...
    ax_bottom_three.plot(
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[2, :, 0],
        tp_to_predict.cpu().numpy(),
        predictions.cpu().numpy()[2, :, 1],
    )
    ax_bottom_three.set_xlim(tp_to_predict.cpu().min(), tp_to_predict.cpu().max())
    ax_bottom_three.set_ylim(
        y_true.cpu()[2, :, :].min() * margin, y_true.cpu()[2, :, :].max() * margin
    )
...
if __name__ == "__main__":
    ...
    (input_dim, output_dim, dltrain, dlval, dltest, _, _, _,) = generate_data_set(
