...
def sample_standard_gaussian(mu, sigma):
  ...
  d = torch.distributions.normal.Normal(
      torch.Tensor([0.0]).to(device).double(),
      torch.Tensor([1.0]).to(device).double())
...
def get_next_batch(dataloader):
  ...
  if ("mask_predicted_data" in data_dict) and (data_dict["mask_predicted_data"]
                                               is not None):
...
def compute_loss_all_batches_direct(
  ...
  if classif:
    ...
    if dataset == "activity":
      ...
      total["accuracy"] = sk.metrics.accuracy_score(class_labels.cpu().numpy(),
